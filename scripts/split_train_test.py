...
Mapping từ tên hiện tại (試料7-1, 試料7-2, 試料6-1, 試料7-4):
  試料7-1 → 7-1-<date>  (class 1)
  試料7-2 → 7-2-<date>  (class 2)
  試料6-1 → 7-3-<date>  (class 3)
  試料7-4 → 7-4-<date>  (class 4)

Cách chạy:
  poetry run python scripts/split_train_test.py
  poetry run python scripts/split_train_test.py --test-ratio 0.2
  poetry run python scripts/split_train_test.py --test-dates 251001 251002 251106 251111
"""
...
# Mapping: tên thư mục hiện tại → (số class, tên mới không có date)
# Tên mới sẽ là 7-<class>-<date>
FOLDER_MAP = {
    "試料7-1": (1, "7-1"),
    "試料7-2": (2, "7-2"),
    "試料6-1": (3, "7-3"),
...
def copy_tree_with_rename(
    ...
    date_suffix = src_date_dir.name  # VD: 250723
    count = 0
    for old_name, (class_num, base_name) in FOLDER_MAP.items():
        src_class_dir = src_date_dir / old_name
        if not src_class_dir.is_dir():
            continue
        new_folder_name = f"{base_name}-{date_suffix}"  # 7-1-250723
